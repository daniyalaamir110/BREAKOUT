...
class Ball(pygame.sprite.Sprite):
    ...
    def update(self) -> None:
        if self.moving == False:
            self.rect.center = Vector2(
                paddle.rect.centerx + self.offset, paddle.rect.centery - 13
            )
        else:
            ...
            if pygame.sprite.collide_rect(self, paddle):
                ...
                self.velocity.y = -math.sqrt(
                    abs(BALL_SPEED**2 - self.velocity.x**2)
                )
    ...
    def isCollidingHorizontally(self, block: pygame.sprite.Sprite) -> bool:
        return (
            self.rect.collidepoint(block.rect.x, block.rect.centery + 5)
            or self.rect.collidepoint(block.rect.x, block.rect.centery - 5)
            or self.rect.collidepoint(block.rect.x, block.rect.centery)
            or self.rect.collidepoint(block.rect.right, block.rect.centery + 5)
            or self.rect.collidepoint(block.rect.right, block.rect.centery - 5)
            or self.rect.collidepoint(block.rect.right, block.rect.centery)
        )
