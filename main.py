...
class FireExplosion(Explosion):
  ...
  def update(self):
    super().update()
    for block in pygame.sprite.spritecollide(
...
class Ball(pygame.sprite.Sprite):
  ...
  def update(self) -> None:
    ...
    if self.moving == False:
      ...
      if pygame.sprite.collide_rect(self, paddle):
        ...
        self.velocity.x = settings.BALL_SPEED * math.sin(2 * offset / paddle.width)
  ...
  def isCollidingHorizontally(self, block: pygame.sprite.Sprite) -> bool:
    return (
      ...
      self.rect.collidepoint(block.rect.right + 9, block.rect.centery - 9))
...
class Brick(Target):
  ...
  def destroy(self):
    if self.life == 2:
      sprites.add(Explosion(
        'PUFF_SMOKE_IMAGES', self.rect.centerx, self.rect.centery))
      self.life = 1
...
class Lava(Target):
  ...
  def update(self) -> None:
    screen.blit(
      assets.LAVA_IMAGES[self.counter], self.rect.topleft)
    self.counter += 1
    if self.counter == 17:
      self.counter = 0
  ...
  def destroy(self) -> None:
    sprites.add(
      FireExplosion(self.rect.centerx, self.rect.centery))
    blocks.remove(self)
    self.kill()
    global score
    score += 100
...
class LevelText(pygame.sprite.Sprite):
  ...
  def __init__(self):
    ...
    self.image = font.render(
      "Level {}".format(level), True, settings.WHITE)
    self.rect = self.image.get_rect()
...
def load_sprites() -> None:
  ...
  balls.add(Ball(
    paddle.rect.centerx, paddle.rect.y - 12))
      ...
      for i in range(len(rows)):
        for j in range(len(rows[i])):
          ...
          if index in range(25):
            blocks.add(Block(assets.BLOCK_IMAGES[index], 48 * j + 12, 24 * i + 12))
          elif char == '1':
...
def __main__() -> None:
  ...
  continue_rect.center = pygame.Vector2(
    settings.CENTER[0], settings.HEIGHT - 300)
  ...
  while running:
    ...
    if not game_started:
      ...
      if paddle.magnet:
        pygame.draw.rect(screen, settings.WHITE, pygame.Rect(
          10, settings.HEIGHT - 20, 302, 10), 1)
        pygame.draw.rect(screen, settings.CYAN, pygame.Rect(
          12, settings.HEIGHT - 18, paddle.magnet // 4, 6))

      if paddle.fires:
        pygame.draw.rect(screen, settings.WHITE, pygame.Rect(
          settings.CENTER[0] - 150, settings.HEIGHT - 20, 302, 10), 1)
        for i in range(paddle.fires):
          pygame.draw.rect(screen, settings.RED, pygame.Rect(
            settings.CENTER[0] - 148 + i * 10, settings.HEIGHT - 18, 8, 6))
